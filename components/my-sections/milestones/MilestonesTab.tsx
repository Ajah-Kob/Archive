...
export function MilestonesTab({ sectionId, initial }: MilestonesTabProps) {
  ...
  async function applyPhase(phase: 'CAPSTONE 1' | 'CAPSTONE 2', open: boolean) {
    ...
    setItems((prev) =>
      prev.map((m) => {
        const keys = phase === 'CAPSTONE 1' ? (CAPSTONE1_KEYS as string[]) : (CAPSTONE2_KEYS as string[])
        if (keys.includes(m.key)) return { ...m, open, openedAt: open ? new Date().toISOString() : null }
        return m
      }),
    )
    toast.success(res.message)
  ...
  const Group = ({ title, phase, phaseItems, allOpen, openCount }: { title: string; phase: 'CAPSTONE 1' | 'CAPSTONE 2'; phaseItems: MilestoneAvailabilityItem[]; allOpen: boolean; openCount: number }) => {
      ...
      <div className="bg-white border border-[#e8ebf8] rounded-[14px] shadow-[0px_2px_12px_0px_rgba(30,58,138,0.06),0px_1px_3px_0px_rgba(0,0,0,0.04)] flex flex-col min-w-0 overflow-hidden">
        <div className="px-5 py-[12px] border-b border-[#f0f2fa] flex items-center justify-between gap-3">
          ...
          <span className="flex items-center gap-2 shrink-0">
            ...
            >
              {isBusy ? <Loader2 className="size-[12px] animate-spin" /> : allOpen ? <Lock className="size-[12px]" /> : <Unlock className="size-[12px]" />}
              {allOpen ? 'Lock' : 'Unlock'}
            </button>
          </span>
        </div>

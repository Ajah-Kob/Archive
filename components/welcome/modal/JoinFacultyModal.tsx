...
export function JoinFacultyModal() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setIsPending(true)
    const res = await joinFaculty(formData)
    setIsPending(false)
    ...
    toast.success("You've joined successfully!", {
      description: 'You now have access to faculty features.',
    })
    await update()
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(16,19,58,0.3)] backdrop-blur-[4px]">
      <div className="relative bg-white border border-[#eceef8] rounded-[16px] w-[420px] p-[29px] shadow-[0px_24px_64px_0px_rgba(16,20,58,0.16),0px_4px_16px_0px_rgba(0,0,0,0.06)] flex flex-col items-start">
        ...
        <form id="join-faculty-form" onSubmit={handleSubmit} className="flex flex-col items-start w-full pt-[22px]">
        ...
        <ModalFooter type="faculty" disabled={isBusy} isPending={isPending} />
      </div>
    </div>
  )
}

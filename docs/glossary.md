...
| **Section slug** | URL-safe form of a section name (`BSIS 4AG1` → `bsis-4ag1`). Used for the coordinator workspace detail route `/faculty/my-section/:sectionId`. |
...
| **Read-only monitoring** | The admin/program-chair view of `/admin/sections` and `/faculty/sections`: search, filter, sort, and drill-down only. No mutations until the coordinator management surface exists. |

...
import { X, FileClock, User, Hash, Globe, Calendar } from 'lucide-react'
...
function DiffHighlight({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <details className="rounded-[10px] border border-[#e8ebf8] bg-white overflow-hidden">
...
export function AuditDetailDrawer({ log, onClose }: AuditDetailDrawerProps) {
  ...
  // Portal guard for SSR
  if (typeof document === 'undefined') return null

  return createPortal(
    <div
      className={`fixed inset-0 z-40 flex justify-end transition-all duration-300 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      aria-hidden={!isOpen}
    >
      <div className="absolute inset-0 bg-[rgba(16,19,58,0.32)] backdrop-blur-[4px]" onClick={onClose} />
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Audit entry details"
        className={`relative w-full max-w-[560px] h-full bg-white border-l border-[#eceef8] shadow-[0_16px_48px_rgba(16,19,58,0.18)] flex flex-col overflow-hidden transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between gap-4 px-6 py-4 border-b border-[#f0f2fa] shrink-0 bg-[#fafbff]/60">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="font-heading font-bold text-[14px] leading-[21px] text-[#10133a] truncate">Audit Details</h2>
              <p className="font-sans text-[12px] leading-[16px] text-[#8a93b4] truncate">
                #{log.id} · {log.action}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="bg-white border border-[#eceef8] rounded-[10px] size-[32px] flex items-center justify-center hover:bg-[#f8f9ff] transition-colors shrink-0"
            >
